...
const Calendar = ({
  ...
  useEffect(() => {
    const calendarElementCurrent = calendarElement.current;
    const handleKeyUp = ({ key }) => {
      /* istanbul ignore else */
      if (key === 'Tab') calendarElementCurrent.classList.remove('-noFocusOutline');
    };
    calendarElementCurrent.addEventListener('keyup', handleKeyUp, false);
    return () => {
      calendarElementCurrent.removeEventListener('keyup', handleKeyUp, false);
    };
  });
  ...
  },[mainState.activeDate, dispatch, isDatepicker])
